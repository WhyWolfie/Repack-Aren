...
//	ZGetGameTypeManager()->SetGameTypeStr( MMATCH_GAMETYPE_CLASSIC_TEAM, ZMsg( MSG_MT_CLASSIC_TEAM));
	...
	ZGetGameTypeManager()->SetGameTypeStr( MMATCH_GAMETYPE_VAMP, ZMsg( MSG_MT_VAMP));
	ZGetGameTypeManager()->SetGameTypeStr( MMATCH_GAMETYPE_CS, ZMsg( MSG_MT_CS));
	ZGetGameTypeManager()->SetGameTypeStr( MMATCH_GAMETYPE_CS_TEAM, ZMsg( MSG_MT_CS_TEAM));
	ZGetGameTypeManager()->SetGameTypeStr( MMATCH_GAMETYPE_SKILLWAR, "SKILL WAR");
	ZGetGameTypeManager()->SetGameTypeStr( MMATCH_GAMETYPE_STAFF, "STAFF");
...
void ZGameInterface::OnDrawStateLogin(MDrawContext* pDC)
{
	...
	if ( pConnectingLabel)
	{
		...
		for ( int i = 0;  i < nCount;  i++)
			szMsg[ i] = '<';
		sprintf( szMsg, "%s %s ", szMsg, "Connecting HEROLATIN");
		for ( int i = 0;  i < nCount;  i++)
			strcat( szMsg, ">");
...
void ZGameInterface::OnDraw(MDrawContext *pDC)
{
	...
	if ( timeGetTime() >= m_dwTimeCount)
	{
		...
		char szText[ 256];
		if ( m_dwHourCount > 3)
			sprintf( szText, "Longer walking playing over %d hours. You're more addicted of GUNZ.", m_dwHourCount);
		else
			sprintf( szText, "Longer walking playing over %d hours.", m_dwHourCount);
		ZChatOutput( MCOLOR(ZCOLOR_CHAT_SYSTEM), szText);


		ZChatOutput( MCOLOR(ZCOLOR_CHAT_SYSTEM), "Thanks for playing GUNZ.");
	}
#endif
...
void ZGameInterface::UpdateBlueRedTeam( void)
{
	...
	// 팀플(팀전)
	if ( m_bTeamPlay && ZGetGameClient()->GetMatchStageSetting()->GetGameType() != MMATCH_GAMETYPE_INFECTED )
	{
