...
*/
	...
	// 임시로 액션리그 
	bool bActionLeague = (strstr(szChannelName,"Action")!=NULL) || (nChannelType==MCHANNEL_TYPE_USER);
...
//	gid = MMUG_DEVELOPER;
	...
	else if ( !ZGetGameClient()->GetRejectNormalChat() ||	// AI¹Y A?Au
...
//	SetBridgePeerFlag(false);
	...
	if (uidChar == GetPlayerUID())
	{
		...
		if((ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_CS) || 
		   (ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_CS_TEAM))
		{
