...
bool ZCombatInterface::OnCreate()
{
	...
	if (ZGetMyInfo()->IsAdminGrade()) {
		...
		if (pCache && pCache->GetUGrade()==MMUG_ADMIN && pCache->CheckFlag(MTD_PlayerFlags_AdminHide)) {
	...
	if (IsShowUI() && !m_bSkipUIDrawByRule)
	{
			...
			&& (ZGetGame()->GetMatch()->GetRoundState() == MMATCH_ROUNDSTATE_PLAY || ZGetGame()->GetMatch()->GetRoundState() == MMATCH_ROUNDSTATE_FINISH))
		{
			...
			if ((!m_Observer.IsVisible() && ZGetGame()->m_pMyCharacter->m_bInfected) || (m_Observer.IsVisible() && m_Observer.GetTargetCharacter()->m_bInfected))
			{
				...
				MBitmap* pBitmapSelection = (RGetIsWidthScreen()) ? m_pInfectedWidescreenOverlay : m_pInfectedOverlay;
			...
			ZGameClient* pGameClient = ZGetGameClient();
					if (pGameClient->GetExtra())
					{
						...
						if (pCharacter && !pCharacter->IsDie())
						{
							...
							TextRelative(pDC, 25.f/800.f, 107.f/600.f, szMsg);
							
							/*sprintf( buf, "%s : %d%%", ZMsg( MSG_CHARINFO_XP), ZGetMyInfo()->GetLevelPercent());
							TextRelative(pDC, 112.f/800.f, 68.f/600.f, szMsg);*/
							...
							//if(ZGetConfiguration()->GetExtra()->bdamage) 
							{
								if(ZGetGame())
									{
										sprintf(szMsg, "DMG: %i", dInfo.nDamageCaused);
										pDC->SetColor(MCOLOR(0xffD00000));
										TextRelative(pDC, 710.f/800.f, 16.f/600.f, szMsg);
									}
								else
									{
									}
			...
			if( ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_INFECTED )
			{
				...
				if (pBmNumLabel)
				{
					...
					if( g_bShowCTFMsg && GetTickCount() - g_dwCTFMsgShowTime < 5000 )
					{
						...
						pDC->SetColor(MCOLOR(0xFF22A4FF));
...
void ZCombatInterface::DrawPont(MDrawContext* pDC)
{
	...
	if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_CS)
	{
		float fRx = (float)MGetWorkspaceWidth()  / 800.0f;
		float fRy = (float)MGetWorkspaceHeight() / 600.0f;
		
		MBitmap* pBitmap = MBitmapManager::Get( "cs-mode.tga");
			if ( pBitmap)
			{
				pDC->SetBitmap( pBitmap);
	...
	if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_CS_TEAM)
	{
		float fRx = (float)MGetWorkspaceWidth()  / 800.0f;
		float fRy = (float)MGetWorkspaceHeight() / 600.0f;
		
		MBitmap* pBitmap = MBitmapManager::Get( "cs-team-mode.tga");
			if ( pBitmap)
			{
				pDC->SetBitmap( pBitmap);
...
void ZCombatInterface::SetPickTarget(bool bPick, ZCharacter* pCharacter)
{
	...
	if (bPick)
	{
		...
		if(pCharacter->IsModeratorName())
			m_pTargetLabel->SetTextColor(ZCOLOR_MODERATOR_NAME);
...
#define CHARACTER_HEIGHT	185.0f
				...
				*/
				...
				else if(pCharacter->IsModeratorName()) {
					pFont = MFontManager::Get("FONTa12_O1Org");
					pDC->SetColor(MCOLOR(ZCOLOR_MODERATOR_NAME));
				}
	...
	if(ZGetGame()->Pick(pTargetCharacter,pos,dir,&pickinfo))
	{
		if (pickinfo.pObject) {
			...
			if (bFriend == false) {
				...
				else if(pPickedCharacter->IsModeratorName()) {
					pDC->SetColor(MCOLOR(ZCOLOR_MODERATOR_NAME));
					pFont = MFontManager::Get("FONTa12_O1Org");
				}
...
typedef list<ZScoreBoardItem*> ZSCOREBOARDITEMLIST;
void ZCombatInterface::DrawScoreBoardCustom(MDrawContext* pDC)
{
//#define TEST_CLAN_SCOREBOARD
	...
	bClanGame = true;
	strcpy(m_szRedClanName, "RED Clan");
	strcpy(m_szBlueClanName, "BLUE clan");
#endif

	ZSCOREBOARDITEMLIST items;

	ZGetScreenEffectManager()->DrawScoreBoardCustom();
	...
	TextRelative(pDC,0.14375f,0.205f,szText);
...
//	const float clanXPOS[6] = {0.45f, 0.25f, 0.68f, 0.77f, 0.83f, 0.92f};
	//1=208-115, 2=376-283, 3=536-443, 4=624-531, 5=672-579, 6=744-651, 7=267,2-174,2, 8=248,8-155,8
	
	//1=352-259, 2=192-99, 3=536-443, 4=608-515, 5=656-563, 6=728-635, 7=411,2-318,2, 8=392,8-299,8
	const float normalXPOS[] = {0.14375f, 0.35375f, 0.55375f, 0.66375f, 0.72375f, 0.81375f, 0.21775f, 0.19475f};
	const float clanXPOS[]   = {0.32375f, 0.12375f, 0.55375f, 0.64375f, 0.70375f, 0.79375f, 0.39775f, 0.37475f};
...
//	int nMaxLineCount=int((fHeight-fTitleHeight)*(float)RGetScreenHeight()/((float)pFont->GetHeight()*1.1f));
	...
	for(i=items.begin();i!=items.end();i++)
	{
		...
		if(pItem->bMyChar || pItem->bCommander)
		{
			...
			//572 + 208 = 780 = 582 + 113
			int x1 = bClanGame ? 0.3125*MGetWorkspaceWidth() : 0.13*MGetWorkspaceWidth();
			int x2 = (0.7275+0.14125)*MGetWorkspaceWidth();
...
void ZCombatInterface::GameCheckPickCharacter()
{
	...
	if(pMyChar && pMyChar->m_pVMesh) {
		...
		if(bPick) {
			pMyChar->m_pVMesh->SetVisibility(0.9f);
		} else {
