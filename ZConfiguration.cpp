	...
	if (!parentElement.IsEmpty())
	{
		...
		if (parentElement.FindChildNode(ZTOK_VIDEO, &childElement))
		{
			...
			childElement.GetChildContents(&m_Video.bTerrible, "NHARDWARETNL");
						// [addon] adjustable FOV
			ZAddons::Francesco::AdjustableFOV::LoadConfig(&childElement, &m_Video);
		...
		if (parentElement.FindChildNode(ZTOK_ETC, &childElement))
		{
			...
			childElement.GetChildContents(&m_Etc.bTrailOption, ZTOK_ETC_TRAIL_OPTION); //Trail Option
...
bool ZConfiguration::SaveToFile(const char *szFileName, const char* szHeader)
{
	...
	// Video
	{
		...
		aElement.SetContents(temp);
				...
				// [addon] adjustable FOV
		ZAddons::Francesco::AdjustableFOV::SaveToFile(&m_Video, temp, &parentElement, &aElement);
...
//	if( parentElement.FindChildNode(ZTOK_MACRO, &childElement) )
	...
	//#endif
		...
		aElement.SetContents(temp);
				...
				//Trail Option
		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_ETC_TRAIL_OPTION);
		sprintf(temp, "%s", m_Etc.bTrailOption?"TRUE":"FALSE");
		aElement.SetContents(temp);
...
void ZConfiguration::Init()
{
	...
	m_Video.nEffectLevel = Z_VIDEO_EFFECT_HIGH;
			// [addon] adjustable FOV
	ZAddons::Francesco::AdjustableFOV::Init(&m_Video);
	...
	m_Macro.SetString(0,"/h");
	m_Macro.SetString(1,"/Provoke");
	m_Macro.SetString(2,"/hi");
	m_Macro.SetString(3,"/wave");
	m_Macro.SetString(4,"Hello!");
	m_Macro.SetString(5,"Let's have a good time and play fair.");
	m_Macro.SetString(6,"Good Shot !!");
	m_Macro.SetString(7,"Thank you very much.");
...
void ZConfiguration::LoadDefaultKeySetting()
{
	...
	static ZACTIONKEYDESCRIPTION DefaultActionKeys[ZACTION_COUNT] = {
		...
		{"HpApShower",   0x43, -1},   //'F9' DOESN'T MATTER!!
