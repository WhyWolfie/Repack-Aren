...
void MMatchRuleTeamSKILLWAR::OnObtainWorldItem(MMatchObject* pObj, int nItemID, int* pnExtraValues)
{
	...
	if(nItemID == CTF_BLUE_ITEM_ID || nItemID == CTF_RED_ITEM_ID )
	{
		...
		unsigned long int nGettingExp = (MMatchFormula::GetGettingExp(pObj->GetCharInfo()->m_nLevel, pObj->GetCharInfo()->m_nLevel) * 1);
		
		MMatchServer::GetInstance()->ApplyObjectTeamBonus(pObj, nGettingExp * 1);
...
MMatchRuleCs::MMatchRuleCs(MMatchStage* pStage) : MMatchRule(pStage)
{

}

void MMatchRuleCs::OnBegin()
{

}
void MMatchRuleCs::OnEnd()
{
}

bool MMatchRuleCs::RoundCount()
{
	if (++m_nRoundCount < 1) return true;
	return false;
}

bool MMatchRuleCs::CheckKillCount(MMatchObject* pOutObject)
{
...
bool MMatchRuleCs::OnCheckRoundFinish()
{
...
void MMatchRuleCs::OnRoundTimeOut()
{
...
MMatchRuleCsTeamDeath::MMatchRuleCsTeamDeath(MMatchStage* pStage) : MMatchRule(pStage)
{
}

void MMatchRuleCsTeamDeath::OnBegin()
{
}

void MMatchRuleCsTeamDeath::OnEnd()
{
}

bool MMatchRuleCsTeamDeath::OnRun()
{
...
void MMatchRuleCsTeamDeath::OnRoundBegin()
{
	MMatchRule::OnRoundBegin();
}

void MMatchRuleCsTeamDeath::OnRoundEnd()
{
	if (m_pStage != NULL)
	{
		switch(m_nRoundArg)
...
bool MMatchRuleCsTeamDeath::OnCheckEnableBattleCondition()
{
	// ¼±½AA|AI °æ¿i´A Free≫oAA°¡ ¾EμE´U.
	...
	int nStageObjects = 0;		// °OAO¾E¿¡ ¾ø°i ½ºA×AIAo¿¡ AO´A ≫c¶÷
...
// ¸¸¾a ·¹μaÆAAI³ª ºi·cÆA¿¡¼­ ÆA¿øAI 0¸iAI °æ¿i´A false ¹YE? , true,false ¸ðμI AliveCount ¹YE?
bool MMatchRuleCsTeamDeath::GetAliveCount(int* pRedAliveCount, int* pBlueAliveCount)
{
	...
	{
		MMatchObject* pObj = (MMatchObject*)(*i).second;
		if (pObj->GetEnterBattle() == false) continue;	// ¹eÆ²Au°¡CI°i AO´A CA·¹AI¾i¸¸ A¼Aⓒ
...
bool MMatchRuleCsTeamDeath::OnCheckRoundFinish()
{
	int nRedAliveCount = 0;
	int nBlueAliveCount = 0;

	// ÆA¿øAI 0¸iAI ÆAAI AOA¸¸e false¹YE?
...
void MMatchRuleCsTeamDeath::OnRoundTimeOut()
{
	int nRedAliveCount = 0;
	int nBlueAliveCount = 0;
...
// ¹YE?°ªAI falseAI¸e °OAOAI ³¡³­´U.
bool MMatchRuleCsTeamDeath::RoundCount() 
{
	...
	else
	{
		...
		// ÆA¿øAI 0¸iAI ÆAAI AO¾iμμ °OAOAI ³¡³­´U.
		...
		// ·¡´o°OAO¿¡¼­ ¸OAu 4½AAI ÆAAI ½A¸®
		const int LADDER_WINNING_ROUNT_COUNT = 4;


		// μIÆAAI ¸ðμI 4½AAI ¾Æ´I¸e true¹YE?
...
void MMatchRuleCsTeamDeath::CalcTeamBonus(MMatchObject* pAttacker, MMatchObject* pVictim,
...
MMatchRuleStaff::MMatchRuleStaff(MMatchStage* pStage) : MMatchRule(pStage)
{

}

void MMatchRuleStaff::OnBegin()
{

}
void MMatchRuleStaff::OnEnd()
{
}

bool MMatchRuleStaff::RoundCount()
{
if (++m_nRoundCount < 1) return true;
return false;
}

bool MMatchRuleStaff::CheckKillCount(MMatchObject* pOutObject)
{
MMatchStage* pStage = GetStage();
for (MUIDRefCache::iterator i=pStage->GetObjBegin(); i!=pStage->GetObjEnd(); i++) 
{
...
bool MMatchRuleStaff::OnCheckRoundFinish()
{
MMatchObject* pObject = NULL;
...
void MMatchRuleStaff::OnRoundTimeOut()
{
SetRoundArg(MMATCH_ROUNDRESULT_DRAW);
}
