...
string MLanguageCode::ConvCodeToStrLangType( const int nLangCode )
{
	...
	if( LANGUAGE_CODE::GRM == nLangCode )
		return string( "grm" );

	if( LANGUAGE_CODE::SPN == nLangCode )
		return string( "spn" );

	if( LANGUAGE_CODE::PRT == nLangCode )
		return string( "prt" );
...
int	MLanguageCode::ConvStrLangTypeToCode( const char* pszLangType )
{
	...
	if( 0 == strcmp("grm", pszLangType) )
		return LANGUAGE_CODE::GRM;

	if( 0 == strcmp("spn", pszLangType) )
		return LANGUAGE_CODE::SPN;

	if( 0 == strcmp("prt", pszLangType) )
		return LANGUAGE_CODE::PRT;
