...
TCHAR g_szDB_GIVE_COINS[] = _T("{CALL spUpdateCoins (%d , %d)}");
TCHAR g_szDB_GIVE_EVENTCOINS[] = _T("{CALL spUpdateEventCoins (%d , %d)}");
...
{ 
    ...
    // °°Aº AI¸§AC A³¸?AI AI¹I A¸AcCI´AAo A¼Aⓒ 
...
{ 
    ...
    try { 
        strSQL.Format(g_szDB_SIMPLE_UPDATE_CHARINFO, pCharInfo->m_nCID, m_DBFilter.Filtering(pCharInfo->m_szName).c_str(), 
...
{ 
    ...
    { 
        mlog( "MMatchDBMgr::InsertConnLog - ¹®AU¿­ IP(%s)ºÐCØ ½CÆÐ. AID(%u)\n",  
            nAID, szIP ); 
        return false; 
    } 
    ...
    try { 
        strSQL.Format( g_szDB_INSERT_CONN_LOG, nAID, vIP[0], vIP[1], vIP[2], vIP[3], m_DBFilter.Filtering(strCountryCode3.c_str()).c_str() ); 
        m_DB.ExecuteSQL( strSQL ); 
    }  
    catch(CDBException* e) { 
...
{ 
    ...
    try { 
        strSQL.Format(g_szDB_INSERT_GAME_LOG, nMasterCID, m_DBFilter.Filtering(szMap).c_str(),  m_DBFilter.Filtering(szGameType).c_str()); 
        rs.Open(strSQL, CRecordset::forwardOnly, CRecordset::readOnly); 
    }  
    catch(CDBException* e) { 
        ExceptionHandler(strSQL, e); 
...
{ 
    ...
    switch (nType) 
    { 
    case CMT_CREATE: strcpy(szType, "≫y¼º"); break; 
    case CMT_DELETE: strcpy(szType, "≫eA|"); break; 
    } 
    ...
    try { 
        strSQL.Format(g_szDB_INSERT_CHAR_MAKING_LOG, nAID, m_DBFilter.Filtering(szCharName).c_str(), szType); 
        m_DB.ExecuteSQL( strSQL ); 
    }  
    catch(CDBException* e) { 
...
{ 
    ...
    try {         
        strSQL.Format(g_szDB_UPDATE_SERVER_INFO, nMaxPlayer, m_DBFilter.Filtering(szServerName).c_str(), nServerID); 
        m_DB.ExecuteSQL( strSQL ); 
    }  
    catch(CDBException* e) { 
...
{ 
    ...
    try {         
        strSQL.Format( g_szDB_UPDATE_LAST_CONNDATE, m_DBFilter.Filtering( szIP ).c_str(), m_DBFilter.Filtering( szUserID ).c_str() ); 
        m_DB.ExecuteSQL( strSQL ); 
    }  
    catch(CDBException* e) { 
...
{ 
    ...
    try    {         
        strSQL.Format(g_szGetCIDbyCharName,  m_DBFilter.Filtering(pszName).c_str()); 
        rs.Open(strSQL, CRecordset::forwardOnly, CRecordset::readOnly); 
    } 
    catch (CDBException* e)    { 
...
{ 
    ...
    CString strSQL; 
    strSQL.Format(g_szDB_GET_CLID_FROM_CLANNAME,  m_DBFilter.Filtering(szClanName).c_str()); 
...
{ 
    ...
    *noutRet = rs.Field("Ret").AsInt(); 
    ...
    /* Ret°ª ¼³¸i : 1 - ¼º°ø, 0 - CØ´cA￢·￡¿øAI ¾ø´U. , 2 - ±CCNAI ¸AAo ¾E´U. */ 
...
{ 
    ...
    CString strSQL; 
    strSQL.Format( g_szGetCIDbyCharName, m_DBFilter.Filtering(pszCharName).c_str() ); 
...
{ 
    ...
    CString strSQL; 
    strSQL.Format(g_szDB_CHECK_PREMIUM_IP, m_DBFilter.Filtering(szIP).c_str()); 
    ...
    /* 2006-08-31AIEA ≫c¿eCIAo ¾EA½. E¤½A ¸ð¸￡´I º¸½A¸e ≫eA| ¹U¶÷. 
    else if ((rs.GetRecordCount() <= 0) || (rs.IsBOF()==TRUE))     // °a°u°ªAI 0°³AI¸e false 
    { 
    outbResult = false; 
    } 
    else 
...
{ 
    ...
    if(sizeof(strEventName) > 24) 
        return false; 

    CString strSQL; 
    strSQL.Format( g_szInsertEvent, dwAID, dwCID, m_DBFilter.Filtering(strEventName.c_str()).c_str() ); 
    try 
    { 
        m_DB.ExecuteSQL( strSQL ); 
    } 
...
{ 
    ...
    { 
        ...
        // IP°¡ ºnA¤≫oAI¾iμμ ·I±×´A ³²°U¾ß CI±a¿¡ 0A¸·I A¼¿oAU. - by SungE 2007-05-04 
        vIP.clear(); 
        vIP.push_back( 0 ); 
        vIP.push_back( 0 ); 
        vIP.push_back( 0 ); 
    ...
    strSQL.Format( g_szSetHackingBlock 
        ...
        , m_DBFilter.Filtering( strEndHackBlockerDate.c_str()).c_str() 
            , nServerID 
            ,  m_DBFilter.Filtering(strChannelName.c_str()).c_str() 
            ,  m_DBFilter.Filtering(strComment.c_str()).c_str() 
            , nBlockLevel ); 
        try 
        { 
            m_DB.ExecuteSQL( strSQL ); 
...
{ 
    ...
    CString strSQL; 
    strSQL.Format( g_szDeleteExpiredClan, dwCLID, dwCID, m_DBFilter.Filtering(strDeleteName.c_str()).c_str(), dwWaitHour ); 
    try 
    { 
        m_DB.ExecuteSQL( strSQL ); 
    } 
...
{ 
    ...
    CString strSQL; 
    strSQL.Format( g_szSetClanDeleteDate, dwMasterCID, dwCLID, m_DBFilter.Filtering(strDeleteDate.c_str()).c_str() ); 
    try 
    { 
        m_DB.ExecuteSQL( strSQL ); 
    } 
...
{ 
    ...
    strSQL.Format( g_spUpdateDTCharacterInfo 
        , dwCID 
        , m_DBFilter.Filtering(szTimeStamp).c_str() 
        , pDTCharInfo->GetTP() 
        , pDTCharInfo->GetWins() 
...
{ 
    ...
    CString strSQL; 
    strSQL.Format( g_spUpdateDTGameLog, m_DBFilter.Filtering(szTimeStamp).c_str(), nLogID, nChampCID ); 
    CODBCRecordset rs(&m_DB); 
...
{ 
    ...
    CString strSQL; 
    strSQL.Format( g_spInsertDTGameLogDetail, nLogID, m_DBFilter.Filtering(szTimeStamp).c_str(), nMatchType, nPlayTime, nWinnerCID, nGainTP, nLoserCID, nLoseTP); 
    CODBCRecordset rs(&m_DB); 
...
{ 
    ...
    try {         
        strSQL.Format(g_spInsertAccountPenaltyInfo, nAID, nPCode, nPenaltyHour, m_DBFilter.Filtering(szGMID).c_str()); 
        m_DB.ExecuteSQL(strSQL); 
    }  
    catch(CDBException* e) { 
...
bool MMatchDBMgr::AdminGiveCoins(const int nAID, const int nCoins)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
...
bool MMatchDBMgr::AdminGiveEventCoins(const int nAID, const int nCoins)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	try {
		strSQL.Format(g_szDB_GIVE_EVENTCOINS, nAID, nCoins);
		m_DB.ExecuteSQL(strSQL);
	}
	catch(CDBException* e){
		ExceptionHandler(strSQL, e);
