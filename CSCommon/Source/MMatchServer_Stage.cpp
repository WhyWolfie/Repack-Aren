...
void MMatchServer::OnPrivateStageJoin(const MUID& uidPlayer, const MUID& uidStage, char* pszPassword)
{
	...
	if (ugid == MMUG_DEVELOPER || ugid == MMUG_ADMIN || ugid == MMUG_MODERATOR || ugid == MMUG_EVENTMASTER || ugid == MMUG_POLICE) 
		bSkipPassword = true;
...
//		pObj->SetInvalidStageSettingDisconnectWaitInfo();
	...
	if ((pChannel) && (bCheckChannelRule))
	{
		...
		if (pRule)
		{
			...
			if ( pNode->nGameType == MMATCH_GAMETYPE_SKILLWAR)
				bCTFMode = true;
			
			if (!pRule->CheckSWRMap(pNode->nMapIndex) && bSWRMode)
			{
				strcpy(pNode->szMapName, MGetMapDescMgr()->GetMapName(MMATCH_MAP_SkillMapV1));
				pNode->nMapIndex = 0;
			}
...
//			OnStageMap(uidStage, pSetting->GetMapName());
		...
		// dude skill war
		else if ( (nLastGameType != MMATCH_GAMETYPE_SKILLWAR) && ( pSetting->GetGameType() == MMATCH_GAMETYPE_SKILLWAR))
		{
			strcpy( szNewMap, MGetMapDescMgr()->GetMapName( MMATCH_MAP_SkillMapV1 ));
			OnStageMap(uidStage, szNewMap);
		}
...
void MMatchServer::OnRequestStageSetting(const MUID& uidComm, const MUID& uidStage)
{
	...
	if (pChar && (MMUG_ADMIN == pChar->GetAccountInfo()->m_nUGrade)) 	{
...
//		}
	...
	if ((pVictim->GetAccountInfo()->m_nUGrade == MMUG_ADMIN) || 
		(pVictim->GetAccountInfo()->m_nUGrade == MMUG_MODERATOR) ||
		(pVictim->GetAccountInfo()->m_nUGrade == MMUG_EVENTMASTER) ||
		(pVictim->GetAccountInfo()->m_nUGrade == MMUG_POLICE) ||
	...
	if ((!bSuicide) &&
		...
		(pAttacker->GetAccountInfo()->m_nUGrade == MMUG_MODERATOR) ||
		(pAttacker->GetAccountInfo()->m_nUGrade == MMUG_EVENTMASTER) ||
		(pAttacker->GetAccountInfo()->m_nUGrade == MMUG_POLICE) ||
...
{	
	//Custom: fix crash overflow w/ player gained exp from stage.
	return;
}
