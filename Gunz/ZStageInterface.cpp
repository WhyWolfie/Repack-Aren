...
void ZStageInterface::ChangeStageGameSetting( const MSTAGE_SETTING_NODE* pSetting)
{
	...
	if ( (pSetting->nGameType == MMATCH_GAMETYPE_DEATHMATCH_SOLO) ||			// 데쓰매치 개인전이거나...
		 ( pSetting->nGameType == MMATCH_GAMETYPE_INFECTED) ||	
		 ...
		 (pSetting->nGameType == MMATCH_GAMETYPE_STAFF) ||
	...
	else if ( pSetting->nGameType == MMATCH_GAMETYPE_VAMP)						// 퀘스트 모드이면...
	{
		// 맵 이름 배경 이미지 변환
		if ( pAniMapImg)
	...
	else if ( pSetting->nGameType == MMATCH_GAMETYPE_CS)						// Au½ºÆ® ¸ðμaAI¸e...
	{
		// ¸E AI¸§ ¹e°æ AI¹IAo º?E?
		if ( pAniMapImg)
	...
	else if ( pSetting->nGameType == MMATCH_GAMETYPE_CS_TEAM)						// Au½ºÆ® ¸ðμaAI¸e...
	{
		// ¸E AI¸§ ¹e°æ AI¹IAo º?E?
		if ( pAniMapImg)
	...
	else if (pSetting->nGameType == MMATCH_GAMETYPE_SKILLWAR)						// Au½ºÆ® ¸ðμaAI¸e...
	{
		// ¸E AI¸§ ¹e°æ AI¹IAo º?E?
		if ( pAniMapImg)
			pAniMapImg->SetCurrentFrame( 7);
