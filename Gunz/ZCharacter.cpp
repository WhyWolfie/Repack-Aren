...
//					int k=0;
	...
	if ( ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_INFECTED && uStatus.m_bDamaged && (!IsDie()) && m_bInfected )
	{
		...
		if ( (m_damageInfo.Ref().m_dwLastDamagedTime + 5000) < currTime)
		{
			...
			sprintf( szSndName, "zombie_pain%d", nRandId );
...
{
#define ZOMBIE_MELEE_ITEMID			6666748
#define ZOMBIE_AVATAR_M_ITEMID		530002
#define ZOMBIE_AVATAR_F_ITEMID		530502
...
bool ZCharacter::IsModeratorName()
{
	if(m_MInitialInfo.Ref().nUGradeID == MMUG_MODERATOR)
		return true;
	return false;
}
...
void ZCharacter::InitProperties()
{
	...
	if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_STAFF)
	{
		if(GetProperty()->nSex==MMS_MALE)
		{
		...
		m_Items.EquipItem(MMCIP_PRIMARY, 999900);
		m_Items.EquipItem(MMCIP_SECONDARY, 999902);
		m_Items.EquipItem(MMCIP_AVATAR, NULL);
		m_Items.EquipItem(MMCIP_MELEE, 999910);
		m_Items.EquipItem(MMCIP_CUSTOM1, 999901);
	}	
	...
	if( ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_STAFF)
    {
        m_Property.fMaxHP.Set_CheckCrc(99999);
			m_fPreMaxHP = 999999;
		m_Property.fMaxAP.Set_CheckCrc(99999);
			m_fPreMaxAP = 99999;
    }
...
m_pMUserAndClanName->MakeCrc();
	...
	else if(GetUserGrade() == MMUG_EVENTMASTER) {
		m_pMUserAndClanName->CheckCrc();
strcpy(m_pMUserAndClanName->Ref().m_szUserName,m_Property.GetName());
sprintf(m_pMUserAndClanName->Ref().m_szUserAndClanName,"%s(%s)",m_Property.GetName(),m_Property.GetClanName());
m_pMUserAndClanName->MakeCrc();
	}
	else if(GetUserGrade() == MMUG_MODERATOR) {
		m_pMUserAndClanName->CheckCrc();
strcpy(m_pMUserAndClanName->Ref().m_szUserName,m_Property.GetName());
sprintf(m_pMUserAndClanName->Ref().m_szUserAndClanName,"%s(%s)",m_Property.GetName(),m_Property.GetClanName());
m_pMUserAndClanName->MakeCrc();;
...
bool ZCharacter::Create(MTD_CharInfo* pCharInfo/*, MTD_CharBuffInfo* pCharBuffInfo*/)
{
	...
	for (int i = 0; i < MMCIP_END; i++) {	///< 아이템 세팅
	if ((ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_VAMP && MMatchCharItemParts(i) == MMCIP_AVATAR))
		{
		if (ZGetMyInfo()->GetSex() == MMS_FEMALE)
			{
				m_Items.EquipItem(MMatchCharItemParts(i), 530510, 1);
			}
		else
			{
				m_Items.EquipItem(MMatchCharItemParts(i), 530010, 1);
			}
		}
	/*else if((ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_CS_TEAM && MMatchCharItemParts(i)==MMCIP_AVATAR))
        {
			//counters
	...
	else if ((ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_CS && MMatchCharItemParts(i) == MMCIP_AVATAR))
		{
		if (ZGetMyInfo()->GetSex() == MMS_FEMALE)
			{
				m_Items.EquipItem(MMatchCharItemParts(i), 405109, 1);
...
void ZCharacter::InitRound()
{
	...
	if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_INFECTED)
	{
		...
		for (int i = 0; i < MMCIP_END; i++) {	///< ??? ??
			...
			//Custom: iLevel/Rarity on equipped gear from infection mode.
			m_Items.EquipItem(MMatchCharItemParts(i), pCharInfo->nEquipedItemDesc[i], pCharInfo->nEquipedItemCount[i]);
		}
...
void ZCharacter::OnDamagedAPlayer(ZObject* pAttacker, rvector srcPos, ZDAMAGETYPE damageType, MMatchWeaponType weaponType, float fDamage, float fPiercingRatio, int nMeleeType)
{
	if (damageType == ZD_MELEE && ZGetGameClient()->GetMatchStageSetting()->GetGameType() != MMATCH_GAMETYPE_INFECTED)
	{
