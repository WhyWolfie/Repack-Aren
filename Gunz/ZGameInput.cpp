...
/*			if ((pMyCharacter) && (pMyCharacter->IsDie()))	//// 실서비스에서 스폰안되는 버그유발. 원인불명(_PUBLISH누락) 영구봉쇄.
	...
	case MWM_ACTIONPRESSED:
		...
		{ 
		if( ZGetGame()->m_pMyCharacter->GetStatus().Ref().bFreezed != 1) 
		{
		switch(pEvent->nKey)
		{
			case ZACTION_FORWARD:
			...
			case ZACTION_HPAP_SHOWER:
				char szName[128];
				{
					...
					{
						sprintf(szName, "!^9 HP: %d / %d AP: %d / %d", (int)ZGetGame()->m_pMyCharacter->GetHP(), (int)ZGetGame()->m_pMyCharacter->GetMaxHP(), (int)ZGetGame()->m_pMyCharacter->GetAP(), (int)ZGetGame()->m_pMyCharacter->GetMaxAP());
