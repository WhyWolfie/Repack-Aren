...
//	m_pLighteningEffect = m_pEffectMeshMgr->Get("ef_lightening");
	...
	//Vamp texture setup
    m_VampMarkList.Create("SFX/Vamp01.tga");
    m_Vamp2MarkList.Create("SFX/Vamp02.tga");
    m_Vamp3MarkList.Create("SFX/Vamp03.tga");
    m_Vamp4MarkList.Create("SFX/Vamp04.tga");
    m_Vamp5MarkList.Create("SFX/Vamp05.tga");
    m_Vamp6MarkList.Create("SFX/Vamp06.tga");
    m_Vamp7MarkList.Create("SFX/Vamp07.tga");
...
void ZEffectManager::Clear()
{
	...
	//vamp declaration
	m_VampMarkList.Clear();
    m_Vamp2MarkList.Clear();
    m_Vamp3MarkList.Clear();
    m_Vamp4MarkList.Clear();
    m_Vamp5MarkList.Clear();
    m_Vamp6MarkList.Clear();
    m_Vamp7MarkList.Clear();
}
...
//			mlog("CreateStateBlock failed : %s",DXGetErrorString(hr));
	...
	if( mode==1 ) {// 물 밖에서만 그린다..
		...
		//Vamp drawing
        m_VampMarkList.Draw();
        m_Vamp2MarkList.Draw();
        m_Vamp3MarkList.Draw();
        m_Vamp4MarkList.Draw();
        m_Vamp5MarkList.Draw();
        m_Vamp6MarkList.Draw();
        m_Vamp7MarkList.Draw();
...
void ZEffectManager::Update(float fElapsed)
{
	...
	//Vamp update
    m_VampMarkList.Update(fElapsed);
    m_Vamp2MarkList.Update(fElapsed);
    m_Vamp3MarkList.Update(fElapsed);
    m_Vamp4MarkList.Update(fElapsed);
    m_Vamp5MarkList.Update(fElapsed);
    m_Vamp6MarkList.Update(fElapsed);
    m_Vamp7MarkList.Update(fElapsed);
...
void ZEffectManager::AddBulletMark(rvector& Target, rvector& TargetNormal)
{
	...
	//Check for Vamp mode
    if(ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_VAMP) {

        switch(rand()%7+1) { //edit 7 to increase Vamp count
                case 1:
                    {
                        m_VampMarkList.Add(Target+TargetNormal, TargetNormal);
                        //nBulletCount++;
                    }
                    break;
                case 2:
                    {
                        m_Vamp2MarkList.Add(Target+TargetNormal, TargetNormal);
                        //nBulletCount++;
                    }
                    break;
                case 3:
                    {
                        m_Vamp3MarkList.Add(Target+TargetNormal, TargetNormal);
                        //nBulletCount++;
                    }
                case 4:
                    {
                        m_Vamp4MarkList.Add(Target+TargetNormal, TargetNormal);
                        //nBulletCount++;
                    }
                    break;
                case 5:
                    {
                        m_Vamp5MarkList.Add(Target+TargetNormal, TargetNormal);
                        //nBulletCount++;
                    }
                    break;
                case 6:
                    {
                        m_Vamp6MarkList.Add(Target+TargetNormal, TargetNormal);
                        //nBulletCount++;
                    }
                    break;
                case 7:
                    {
                        m_Vamp7MarkList.Add(Target+TargetNormal, TargetNormal);
                        //nBulletCount++;
                    }
                    break;
            }
