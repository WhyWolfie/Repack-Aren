...
void ZScreenEffectManager::Destroy()
{
	...
	SAFE_DELETE(m_pScorePanelCustom);
...
#define END_(x) { _end_time = timeGetTime(); float f_time = (_end_time - _begin_time) / 1000.f; mlog("%s : %f \n", x,f_time ); }
	...
	m_pScorePanelCustom	= new ZScreenEffect(m_pEffectMeshMgr->Get("ef_in_tab_Custom.elu"));
...
void ZScreenEffectManager::DrawScoreBoardCustom()
{
	m_pScorePanelCustom->Draw(0);
}
