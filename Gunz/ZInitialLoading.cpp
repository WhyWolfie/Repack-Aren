...
#ifdef LOCALE_KOREA
	if ( (rand() % 4) == 0)
		sprintf( m_szNotice, "Private Server GUNZ.");
	/*else if
		sprintf( m_szNotice, "You are entering GUNZ1.");
	else if
		sprintf( m_szNotice, "You are entering GUNZ2.");*/
	else
		sprintf( m_szNotice, "You are entering GUNZ.");
...
void	ZInitialLoading::Draw( LOADING_SCENE_MODE mode_ /* = MODE_DEFAULT */, int destIndex_ /* = -1 */, bool bFlip_, bool fromThread_ )
{
	...
	//	if( mode_ == MODE_FADEOUT )
		...
		if(m_pLoadingStr && mpDC ) {
			char buffer[256];
			sprintf(buffer,"Loading %s GUNZ...",m_pLoadingStr);
