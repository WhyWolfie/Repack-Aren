		...
		case MC_ADMIN_FREEZE:
			{
				...
				ZGetGame()->m_pMyCharacter->GetStatus().Ref().bFreezed = !ZGetGame()->m_pMyCharacter->GetStatus().Ref().bFreezed;
				ZGetGame()->m_pMyCharacter->GetStatus().MakeCrc();

				sprintf(szOutput, "You have been %sfrozen by %s.", ((ZGetGame()->m_pMyCharacter->GetStatus().Ref().bFreezed) ? "" : "un"), szAdminName);
