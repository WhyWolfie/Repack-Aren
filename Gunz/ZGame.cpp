...
//		ZPostRequestGameInfo(ZGetGameClient()->GetPlayerUID(), ZGetGameClient()->GetStageUID());
	...
	ZAddons::Francesco::AdjustableFOV::TrySetConfiguredFOV();
...
//	else if(gid == MMUG_BLOCKED)		{ UserNameColor = MCOLOR(200,200,200); return true; }// 사용정지
	...
	{ 
		...
		UserNameColor = MCOLOR(0,218, 255); 
		/* if(sp_name) { 
	...
	else if(gid == MMUG_MODERATOR) {
		UserNameColor = MCOLOR(255,125, 125); 
		/* if(sp_name) { 
			strcpy(sp_name,ZMsg(MSG_WORD_MODERATOR));
		} */
		return true; 
	}
	
	else if(gid == MMUG_VIP1) {
		UserNameColor = MCOLOR(59,144, 240); 
		/* if(sp_name) { 
			strcpy(sp_name,ZMsg(MSG_WORD_MODERATOR));
		} */
		return true; 
		...
		UserNameColor = MCOLOR(59,240, 134); 
		/* if(sp_name) { 
			strcpy(sp_name,ZMsg(MSG_WORD_MODERATOR));
		} */
		return true; 
		...
		UserNameColor = MCOLOR(240,207, 59); 
		/* if(sp_name) { 
			strcpy(sp_name,ZMsg(MSG_WORD_MODERATOR));
		} */
		return true; 
		...
		UserNameColor = MCOLOR(213,59, 240); 
		/* if(sp_name) { 
			strcpy(sp_name,ZMsg(MSG_WORD_MODERATOR));
		} */
		return true; 
...
{
	const char *testdeathnametable[ZD_END+1] = { "Fail", "Pistol", "Dagger", "Crash", "Explosion", "HEADSHOT", "Last Shank" };
	char szMsg[256] = "";
	...
	if (GetMatch()->GetMatchType() == MMATCH_GAMETYPE_INFECTED)
	{
		...
		if (pVictim == pAttacker && pVictim->GetTeamID() == MMT_BLUE)
			sprintf( szInfectedMsg, "Humano '%s' ha muerto!", pVictim ? pVictim->GetUserName() : "Unknown" );
		else if (pVictim->GetTeamID() == MMT_RED)
		{
			sprintf( szInfectedMsg, "Zombie '%s' ha sido asesinado!", pVictim ? pVictim->GetUserName() : "Unknown" );
			ZGetSoundEngine()->PlaySound( "zombie_die" );
		}
		else if (pVictim->GetTeamID() == MMT_BLUE)
			sprintf( szInfectedMsg, "Humano '%s' ha sido infectado!", pVictim ? pVictim->GetUserName() : "Unknown" );
	...
	if(pAttacker==pVictim)
	{
		if (pVictim == m_pMyCharacter)
		{
			if(m_pMyCharacter->GetLastDamageType()==ZD_EXPLOSION) {
				sprintf(szMsg, "You have committed suicide.");
		...
		{
			sprintf(szMsg, "%s has commited suicide.", szAttacker);
			...
			if (ZGetMyInfo()->IsAdminGrade()) {
				...
				{
					sprintf( szMsg, "^%d%s^9 has commited suicide.",
	...
	{
		sprintf(szMsg, "You have defeated %s.", szVictim );
...
//		sprintf(szMsg, "당신은 %s님에게 패배하였습니다.", szAttacker );
		...
		// Custom: Added HP to death message, and game must not be a clan war
	...
	{
		sprintf(szMsg, "%s has defeated %s.", szAttacker, szVictim );
...
{
	//if (ZGetMyInfo()->IsAdminGrade())
	if (ZGetMyInfo()->GetUGradeID() == MMUG_POLICE)
	{
	...
	case MMATCH_ROUNDSTATE_PLAY:
		{
			...
			else if ((GetMatch()->GetMatchType() == MMATCH_GAMETYPE_CS) || (GetMatch()->GetMatchType() == MMATCH_GAMETYPE_CS_TEAM))
            {
				ZGetGameInterface()->PlayVoiceSound( VOICE_GOGOGO, 2100);
	...
	case MMATCH_ROUNDSTATE_FINISH:
		{
			...
			if (m_Match.IsTeamPlay())
			{
				...
				else 
				{
					if (nArg == MMATCH_ROUNDRESULT_DRAW)
					{
						...
						else if (GetMatch()->GetMatchType() == MMATCH_GAMETYPE_CS_TEAM)
						{
							if ( nTeamWon == MMT_RED)
								ZGetGameInterface()->PlayVoiceSound( VOICE_TERWIN, 2800);
							else
								ZGetGameInterface()->PlayVoiceSound( VOICE_CTWIN, 2800);
						}
						else
						{
							if ( nTeamWon == MMT_RED)
			...
			else if( ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DUELTOURNAMENT )
			{
				if(!ZGetCombatInterface()->GetObserver()->IsVisible())	// 옵져버가 아니면
				{
					...
					sprintf(szAccumulationDamagePrint, "Cumulative damage[%2.1f] server sends", fAccumulationDamage);
