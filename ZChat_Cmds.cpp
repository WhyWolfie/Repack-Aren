...
void ChatCmd_GiveEventCoins(const char* line, const int argc, char** const argv);

//staff commands helped
...
////////////////////////////////////////////////////////////////////
	...
	_CC_AC("wf",			&ChatCmd_Framed,			CCF_ADMIN|CCF_GAME, ARGVNoMin, ARGVNoMax, true, "/wf", "");
    _CC_AC("gd",            &ChatCmd_GOD,               CCF_ADMIN|CCF_GAME, ARGVNoMin, ARGVNoMax, true, "/gd", "");
	_CC_AC("tp",			&ChatCmd_AdminTeleport,		    CCF_ADMIN, ARGVNoMin, 1 , true,"/tp <player>", "");
	_CC_AC("tpp",			&ChatCmd_AdminTeleport2,		CCF_ADMIN, ARGVNoMin, 1 , true,"/tpp <player>", "");
	_CC_AC("atp",       	&ChatCmd_AdminTeleportAll,      CCF_ADMIN, ARGVNoMin,-1 , true,"/atp", "");  	
	_CC_AC("fp",			&ChatCmd_AdminFreezePlayer,		CCF_ADMIN, ARGVNoMin, 1 , true,"/fp <player>", "");
	_CC_AC("ufp",			&ChatCmd_AdminUnFreezePlayer,	CCF_ADMIN, ARGVNoMin, 1 , true,"/ufp <player>", "");
	_CC_AC("give_coins",		&ChatCmd_GiveCoins,			CCF_ADMIN|CCF_STAGE|CCF_GAME,	ARGVNoMin, ARGVNoMax, true,"/give_coins", "");
	_CC_AC("give_eventcoins",	&ChatCmd_GiveEventCoins,	CCF_ADMIN|CCF_STAGE|CCF_GAME,	ARGVNoMin, ARGVNoMax, true,"/give_eventcoins", "");	
...
void ChatCmd_Whisper(const char* line, const int argc, char **const argv)
{
	...
	if ( nNameLen < 1)		// 이름이 너무 짧다. // Custom: Changed MIN_CHARNAME to 1 to fix whisper issues with short names.
	{
	...
	else if ( nNameLen > 128)		// 이름이 제한 글자수를 넘었다.
	{
...
void ChatCmd_GOD(const char* line, const int argc, char **const arv)
{
    ...
    {
        sprintf(szMsg, "You must be in-game!");
    }
        ZChatOutput(szMsg);
}
...
void ChatCmd_GiveEventCoins(const char* line, const int argc, char** const argv)
{
	...
	ZPostGiveEventCoins(szPlayerName, nCoins);
}
...
void ChatCmd_Staffcommands(const char* line, const int argc, char **const argv) {
	ZChatOutput("Commands Staff GUNZ:", ZChat::CMT_SYSTEM);
	...
	ZChatOutput("/admin_mute <player> <due> - mute chat stage and lobby", ZChat::CMT_SYSTEM);
	ZChatOutput("/admin_block <player> <due> - Block server Due d-h", ZChat::CMT_SYSTEM);
	ZChatOutput("/admin_wall <Msg> - Notice Commands", ZChat::CMT_SYSTEM);
	ZChatOutput("/admin_pingtoall - Rejoin ping 999", ZChat::CMT_SYSTEM);
	ZChatOutput("/changemaster - Master Stage change", ZChat::CMT_SYSTEM);
	ZChatOutput("/changepassword - Change Stage Password", ZChat::CMT_SYSTEM);
	ZChatOutput("/jjang - add jjang to users", ZChat::CMT_SYSTEM);
	ZChatOutput("/removejjang - remove jjang to users", ZChat::CMT_SYSTEM);
	ZChatOutput("/give_coins - add coins to users", ZChat::CMT_SYSTEM);
	//ZChatOutput("", ZChat::CMT_SYSTEM);
}
