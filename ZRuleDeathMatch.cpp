...
////////////////////
ZRuleCs::ZRuleCs(ZMatch* pMatch) : ZRule(pMatch)
{
...
ZRuleCsTeamDeathMatch::ZRuleCsTeamDeathMatch(ZMatch* pMatch) : ZRule(pMatch)
{

}

ZRuleCsTeamDeathMatch::~ZRuleCsTeamDeathMatch()
{
...
ZRuleStaff::ZRuleStaff(ZMatch* pMatch) : ZRule(pMatch)
{

}

ZRuleStaff::~ZRuleStaff()
{
		...
		case MC_MATCH_SKILFLAG_CAP:
		{
			...
			if(pCapper)
			{
				if(!pCapper->IsDie())
				{
					...
					sprintf( szMsg,"%s You Winner!", pCapper->GetUserAndClanName() );
	...
	case MC_MATCH_LASTSURVIVOR:
		{
			...
			if (pChar)
			{
				...
				{
					sprintf( szInfectedMsg, "Usted es el ultimo sobreviviente." );
					ZGetGameInterface()->GetCombatInterface()->UpdateCTFMsg( szInfectedMsg );
				}
				else
				{
					sprintf( szInfectedMsg, "'%s' es el ultimo sobreviviente.", pChar->GetUserName() );
					ZGetGameInterface()->GetCombatInterface()->UpdateCTFMsg( szInfectedMsg );
				}
			}
...
void ZRuleTeamInfected::OnInfected(const MUID& uidPlayer, bool bFirst)
{
	...
	if(pChar)
	{
		...
		sprintf( szSndName, "zombie_voice_idle%d", nRandId );
		...
		if ((!ZGetGameInterface()->GetCombatInterface()->GetObserverMode() && pChar == ZGetGame()->m_pMyCharacter) || (ZGetGameInterface()->GetCombatInterface()->GetObserverMode() && pChar == ZGetGameInterface()->GetCombatInterface()->GetObserver()->GetTargetCharacter()))
		{
			...
			sprintf( szInfectedMsg, "Usted esta infectado. ¡HA MATAR A TODOS!" );
...
void ZRuleTeamInfected::OnSetRoundState( MMATCH_ROUNDSTATE roundState )
{
	...
	case MMATCH_ROUNDSTATE_PRE_COUNTDOWN:
		{
			ZGetGameInterface()->GetCombatInterface()->UpdateCTFMsg("Los zombies se generan pronto. ¡ESCONDETE!");
			m_bLastFive = false;
	...
	case MMATCH_ROUNDSTATE_PLAY:
		{
			...
			ZGetGameInterface()->GetCombatInterface()->UpdateCTFMsg("¡Ha Correr!");
			ZChatOutput("¡Ha Correr!", ZChat::CMT_SYSTEM);
		}
		break;

	case MMATCH_ROUNDSTATE_FREE:
		{
			ZGetGameInterface()->GetCombatInterface()->UpdateCTFMsg("Se requieren 4 o mas jugadores para iniciar el juego.");
			m_bLastFive = false;
			m_uidPatientZero = MUID( 0, 0 );
